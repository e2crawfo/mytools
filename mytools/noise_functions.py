import numpy as np
import hrr
import random

def default_noise(vec):
    vector = np.random.rand(len(vec)) - 0.5
    vector = vector / np.linalg.norm(vector)
    return vector

def flip_noise(vec):
    return -vec

def ortho_vector(input_vec, normalize=False):
    """
    Returns a random vector orthogonal to the input vector.

    Args:
    input_vec -- Returned vector will be orthogonal to input_vec
    normalize -- whether to normalize the vector before returning
    """

    dim = len(input_vec)
    ortho = hrr.HRR(dim).v
    proj = np.dot(input_vec, ortho) * input_vec
    ortho -= proj
    assert np.allclose([np.dot(ortho, input_vec)], [0])
    if normalize:
        ortho = ortho / np.linalg.norm(ortho)
    return ortho

def make_hrr_noise(D, N, normalize=False):
    """
    Returns a function. That function accepts a vector and returns
    a noisy version of the vector that results from extracting this vector
    from an HRR paired associates memory.

    Args:
    D -- the dimension of all vectors involved
    N -- the number of other pairs in the HRR vector that is created
    normalize -- whether to normalize the HRR vector before deconvolving
    """

    def hrr_noise(input_vec):
        """
        Accepts a numpy vector as input, returns a version of that vector corrupted
        by HRR-style noise (i.e. when the vector is extracted from an HRR)
        """
        noise_vocab = hrr.Vocabulary(D)
        keys = [noise_vocab.parse(str(x)) for x in range(2*N+1)]

        input_vec = hrr.HRR(data=input_vec)
        partner_key = random.choice(keys)

        pair_keys = filter(lambda x: x != partner_key, keys)

        pairs = random.sample(pair_keys, 2 * N)
        p0 = (pairs[x] for x in range(0,len(pairs),2))
        p1 = (pairs[x] for x in range(1,len(pairs),2))
        expression = map(lambda x, y: noise_vocab[x].convolve(noise_vocab[y]), p0, p1)

        expression = reduce(lambda x, y: x + y, S, noise_vocab[partner_key].convolve(input_vec))

        if normalize:
            expression.normalize()

        vec_hrr = expression.convolve(~noise_vocab[partner_key])
        return vec_hrr.v

    return hrr_noise

def make_hrr_noise_from_string(D, expression, names={}, normalize=False, verbose=False):
    """
    Returns a function that takes an input vector and returns a version of that vector
    corrupted by noise. The noise is generated by first constructing an HRR vector
    from the string expression which includes in the input vector, and then performing
    the appropriate operations to extract the input vector from that expression.

    expression is assumed to specify superpositions of convolutions of HRR vectors.
    The convolved vectors can also be inverted.

    Special subsstrings:
    '!' : Must include exactly one instance. Acts as a placeholder for input vector.
    '?' : Each instance of '?' is replaced by a random HRR vector.
    '?u' : Each instance of '?u' is replaced by a random unitary HRR vector.

    Vectors filling '?' and '?u' are generated anew for each call of the returned function.

    Some examples of valid strings:

    '? * ! + ? * ?'
    'a * b * c + a * a * !'
    '? * a * !'
    'b * ! * b + c * c * c * ~c + d * ~d'

    Args:

    D -- dimension of all vectors.

    expression -- a string giving the form of the HRR expression to extract from.

    names -- a dictionary mapping strings to HRRs. Can be used to specify values
             for variables that occur in expression. Any variable name that does not occur
             in names will be assigned a random HRR.

    normalize -- whether to normalize the HRR expression before deconvolving.
    """

    original = expression

    placeholder = 'p0'
    expression = expression.replace('!', placeholder)

    num_unitary_wildcards = expression.count('?u')
    expression = expression.replace('?u', '%s')
    unitary_names = ['u'+str(i) for i in range(num_unitary_wildcards)]
    expression = expression % tuple(unitary_names)

    num_wildcards = expression.count('?')
    expression = expression.replace('?', '%s')
    temp_names = ['h'+str(i) for i in range(num_wildcards)]
    expression = expression % tuple(temp_names)

    terms = expression.split('+')

    #Find the placeholder - assuming superposition of convolutions
    query_vectors = None
    for term in terms:
        if placeholder in term:
            query_vectors = term.split('*')
            query_vectors = [qv.strip() for qv in query_vectors]
            query_vectors.remove(placeholder)
            break

    if query_vectors is None:
        raise ValueError('HRR string must contain the placeholder: ' + placeholder)


    if verbose:
        print 'Evaluated expression: ', expression
        print 'Extraction expression: ', 'h * ~(' + ' * '.join(query_vectors) + ')'

    def hrr_noise_from_string(input_vec):
        """
        Uses expression, names, query_vectors from wrapper function

        Args:

        input_vec -- the vector to add noise to. Can be an HRR vector or a numpy ndarry.
                     Returns a noisy vector of the same type is input_vec.

        """

        use_ndarray = type(input_vec) == np.ndarray
        if use_ndarray:
            input_vec = hrr.HRR(data=input_vec)


        vocab = hrr.Vocabulary(D, unitary=unitary_names)

        for n, v in names.iteritems():
            vocab.add(n, v)

        vocab.add(placeholder, input_vec)

        try:
            h = eval(expression, {}, vocab)
        except Exception as e:
            print 'Error evaluating HRR string ' + original
            raise e

        if normalize:
            h.normalize()

        vocab.add('h', h)

        noisy = eval('h * ~(' + ' * '.join(query_vectors) + ')', {}, vocab)

        if use_ndarray:
            noisy = noisy.v

        return noisy

    return hrr_noise_from_string


def output(trial_length, main, main_vector, alternate, noise_func=default_noise):
    tick = 0
    vector = main_vector
    main_hrr = hrr.HRR(data=main_vector)

    while True:
        if tick == trial_length :
            tick = 0
            if main:
                vector = main_vector
            else:
                vector = noise_func(main_vector)
                u = hrr.HRR(data=vector)
                similarity = u.compare(main_hrr)
                print "Sim:", similarity

            if alternate:
                main = not main

        tick += 1

        yield vector

def interpolator(end_time, start_vec, end_vec, time_func=lambda x: x):
    tick = 0
    while True:
        t = time_func(tick)
        t = min(t, end_time)

        vector = (end_time - t) * start_vec + t * end_vec
        vector = vector / np.linalg.norm(vector)
        tick += 1

        yield vector


def make_f(generators, times):
    last_time = [0.0]
    def f(t):
        if len(generators) > 1 and t > times[0] + last_time[0]:
            generators.pop(0)
            last_time[0] += times.pop(0)
        return generators[0].next()
    return f

if __name__ == "__main__":
    D = 512
    i = hrr.HRR(D)
    #f = make_hrr_noise_from_string(D, '?*?*?*!*?')
    f = make_hrr_noise_from_string(D, '?u*?u*?*!*?u')
    x= f(i)
    print i.compare(x)

    i = hrr.HRR(D)
    f = make_hrr_noise_from_string(D, '?*? *? + ? * ? * ? + ? * ? * ! + ?*?', normalize=False)
    x= f(i)
    print i.compare(x)

    i = hrr.HRR(D)
    f = make_hrr_noise_from_string(D, '? * ! + ? * ?', normalize=False)
    x = f(i)
    print i.compare(x)

    i = hrr.HRR(D)
    f = make_hrr_noise_from_string(D, 'a * b * c + a * a * !', normalize=False)
    x = f(i)
    print i.compare(x)

    i = hrr.HRR(D)
    f = make_hrr_noise_from_string(D, '? * a * !', normalize=False)
    x = f(i)
    print i.compare(x)

    i = hrr.HRR(D)
    f = make_hrr_noise_from_string(D, 'b * ! * b + c * c * c * ~c + d * ~d', normalize=False)
    x = f(i)
    print i.compare(x)

